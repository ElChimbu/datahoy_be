...
app.get('/api/pages/*', async (req: Request, _res: Response, next) => {
  const fullPath = req.path;

  // Skip if this is exactly /api/pages or starts with /api/pages/id/
  if (fullPath === '/api/pages' || fullPath.match(/^\/api\/pages\/id\//)) {
    return next();
  }
  ...
  if (slug) {
    ...
    const { PagesController } = await import('./controllers/pages.controller.js');
    return PagesController.getPageBySlug(req, _res);
  }
